...
function VillamentModule({ action, onDataUpdate }) {
  ...
  const fetchProperty = async (listingId, action) => {
    try {
      ...
      if (response.data.status === "success") {
        ...
        setCallForPrice(listingData.ltg_det_villaments_call_for_price);
  ...
  useEffect(() => {
    handleDataUpdate();
  }, [salePrice, suffixPrice, callForPrice, areaDetails, ratePerSqFt, content, locationData, selectedTriplex, totalFloors,
