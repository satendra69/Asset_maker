...
function PropertyVilla() {
  ...
  const [propertiesVilla, setPropertiesVilla] = useState([]);
  ...
  const fetchProperties = async (filterParams = {}) => {
    try {
      ...
      console.log("response____Propperty111", response);

      if (response.data.status === "success") {
        setPropertiesVilla(response.data.data);
      }
    } catch (error) {
