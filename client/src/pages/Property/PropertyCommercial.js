...
function PropertyCommercial() {
  ...
  const [propertiesCommercial, setPropertiesCommercial] = useState([]);
  const { filters } = useParams();

  const type = "CommercialProperties";
  ...
  const fetchProperties = async (filterParams = {}) => {
    try {
      ...
      if (response.data.status === "success") {
        setPropertiesCommercial(response.data.data);
      }
    } catch (error) {
...
export default PropertyCommercial;
